...
func (t *TeeStore) Put(ctx context.Context, mode storage.ModePut, chs ...infinity.Chunk) (exist []bool, err error) {
...
// Put implements storage.Putter.
func (f *FsStore) Put(ctx context.Context, mode storage.ModePut, chs ...infinity.Chunk) (exist []bool, err error) {
	for _, ch := range chs {
...
func (f *FsStore) Get(ctx context.Context, mode storage.ModeGet, address infinity.Address) (ch infinity.Chunk, err error) {
	chunkPath := filepath.Join(f.path, address.String())
	data, err := ioutil.ReadFile(chunkPath)
	if err != nil {
		return nil, err
	}
	return infinity.NewChunk(address, data), nil
}

// ApiStore provies a storage.Putter that adds chunks to Smart Chain through the HTTP chunk API.
...
// Put implements storage.Putter.
func (a *ApiStore) Put(ctx context.Context, mode storage.ModePut, chs ...infinity.Chunk) (exist []bool, err error) {
	for _, ch := range chs {
...
// Get implements storage.Getter.
func (a *ApiStore) Get(ctx context.Context, mode storage.ModeGet, address infinity.Address) (ch infinity.Chunk, err error) {
	...
	ch = infinity.NewChunk(address, chunkData)
	return ch, nil
}
