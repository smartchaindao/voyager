...
func TestFsStore(t *testing.T) {
	...
	chunkAddr := infinity.MustParseHexAddress(hashOfFoo)
	chunkData := []byte("foo")
	ch := infinity.NewChunk(chunkAddr, chunkData)
	...
	chResult := infinity.NewChunk(chunkAddr, chunkDataResult)
	if !ch.Equal(chResult) {
		t.Fatal("chunk mismatch")
	}
}
...
// newTestServer creates an http server to serve the voyager http api endpoints.
