...
APP_NAME="voyager"
REPO_URL="https://github.com/yanhuangpai/voyager"

: "${USE_SUDO:="true"}"
: "${VOYAGER_INSTALL_DIR:="/usr/local/bin"}"
...
# check_installed_version checks which version of voyager is installed and
# if it needs to be changed.
check_installed_version() {
  if [[ -f "${VOYAGER_INSTALL_DIR}/${APP_NAME}" ]]; then
    local version=$(voyager version 2>&1)
    if [[ "${version%-*}" == "${TAG#v}" ]]; then
      echo "voyager ${version} is already ${DESIRED_VERSION:-latest}"
      return 0
    else
      echo "voyager ${TAG} is available. Changing from version ${version}."
      return 1
    fi
  else
    return 1
...
# for that binary.
download_file() {
  VOYAGER_DIST="voyager-$OS-$ARCH"
  if [ "$OS" == "windows" ]; then
    VOYAGER_DIST="voyager-$OS-$ARCH.exe"
  fi
  DOWNLOAD_URL="$REPO_URL/releases/download/$TAG/$VOYAGER_DIST"
  VOYAGER_TMP_ROOT="$(mktemp -dt voyager-binary-XXXXXX)"
  VOYAGER_TMP_FILE="$VOYAGER_TMP_ROOT/$VOYAGER_DIST"
  if command -v curl &> /dev/null; then
    curl -SsL "$DOWNLOAD_URL" -o "$VOYAGER_TMP_FILE"
  elif command -v wget &> /dev/null; then
    wget -q -O "$VOYAGER_TMP_FILE" "$DOWNLOAD_URL"
  fi
}
...
# installs it.
install_file() {
  echo "Preparing to install $APP_NAME into ${VOYAGER_INSTALL_DIR}"
  run_as_root chmod +x "$VOYAGER_TMP_FILE"
  run_as_root cp "$VOYAGER_TMP_FILE" "$VOYAGER_INSTALL_DIR/$APP_NAME"
  echo "$APP_NAME installed into $VOYAGER_INSTALL_DIR/$APP_NAME"
}
...
test_binary() {
  if ! command -v $APP_NAME &> /dev/null; then
    echo "$APP_NAME not found. Is $VOYAGER_INSTALL_DIR on your "'$PATH?'
    exit 1
  fi
...
cleanup() {
  if [[ -d "${VOYAGER_TMP_ROOT:-}" ]]; then
    rm -rf "$VOYAGER_TMP_ROOT"
  fi
}
