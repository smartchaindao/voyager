...
// message:
func Wrap(ctx context.Context, topic Topic, msg []byte, recipient *ecdsa.PublicKey, targets Targets) (infinity.Chunk, error) {
	...
	enc, ephpub, err := elgamal.NewEncryptor(recipient, topic[:], 4032, infinity.NewHasher)
	if err != nil {
		return nil, err
	}
...
func mine(ctx context.Context, odd bool, f func(nonce []byte) (infinity.Chunk, error)) (infinity.Chunk, error) {
	seeds := make([]uint32, 8)
	for i := range seeds {
	...
	result := make(chan infinity.Chunk, 8)
...
func matchTopic(key *ecdsa.PrivateKey, pubkey *ecdsa.PublicKey, hint []byte, topic []byte) (encryption.Decrypter, error) {
	dec, err := elgamal.NewDecrypter(key, pubkey, topic, infinity.NewHasher)
	if err != nil {
		return nil, err
	}
