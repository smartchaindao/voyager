...
func TestNewCAC(t *testing.T) {
	...
	address := infinity.MustParseHexAddress(bmtHashOfData)

	expectedSpan := make([]byte, infinity.SpanSize)
...
func TestChunkInvariants(t *testing.T) {
	chunkerFunc := []struct {
		name    string
		chunker func(data []byte) (infinity.Chunk, error)
	}{
		{
			name:    "new cac",
			chunker: cac.New,
	...
	for _, f := range chunkerFunc {
		for _, cc := range []struct {
			...
			},
			{
				...
				data:    []byte(strings.Repeat("a", infinity.ChunkSize+infinity.SpanSize+1)),
...
func TestValid(t *testing.T) {
	...
	address := infinity.MustParseHexAddress(bmtHashOfFoo)

	foo := "foo"
	fooLength := len(foo)
	fooBytes := make([]byte, infinity.SpanSize+fooLength)
	binary.LittleEndian.PutUint64(fooBytes, uint64(fooLength))
	copy(fooBytes[infinity.SpanSize:], foo)
	ch := infinity.NewChunk(address, fooBytes)
	if !cac.Valid(ch) {
