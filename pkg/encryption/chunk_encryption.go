...
func newSpanEncryption(key Key) Interface {
	refSize := int64(infinity.HashSize + KeyLength)
	return New(key, 0, uint32(infinity.ChunkSize/refSize), sha3.NewLegacyKeccak256)
}

func newDataEncryption(key Key) Interface {
	return New(key, int(infinity.ChunkSize), 0, sha3.NewLegacyKeccak256)
}
