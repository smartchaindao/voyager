...
func newService(t *testing.T, networkID uint64, o libp2pServiceOpts) (s *libp2p.Service, overlay infinity.Address) {
	t.Helper()
	...
	overlay, err = crypto.NewOverlayAddress(infinityKey.PublicKey, networkID)
	if err != nil {
		t.Fatal(err)
	}
	...
	s, err = libp2p.New(ctx, crypto.NewDefaultSigner(infinityKey), networkID, overlay, addr, o.Addressbook, statestore, o.Logger, nil, o.libp2pOpts)
	if err != nil {
		t.Fatal(err)
	}
	s.Ready()
...
func expectPeers(t *testing.T, s *libp2p.Service, addrs ...infinity.Address) {
	t.Helper()
...
func expectPeersEventually(t *testing.T, s *libp2p.Service, addrs ...infinity.Address) {
	t.Helper()
