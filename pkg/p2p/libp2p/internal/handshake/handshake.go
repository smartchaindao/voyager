...
var (
	...
	// ErrHandshakeDuplicate is returned  if the handshake response has voyagern received by an already processed peer.
...
func (s *Service) Handshake(ctx context.Context, stream p2p.Stream, peerMultiaddr ma.Multiaddr, peerID libp2ppeer.ID) (i *Info, err error) {
	...
	remoteIfiAddress, err := s.parseCheckAck(resp.Ack)
	if err != nil {
		return nil, err
	}
	...
	ifiAddress, err := ifi.NewAddress(s.signer, advertisableUnderlay, s.overlay, s.networkID)
	if err != nil {
		return nil, err
	}

	advertisableUnderlayBytes, err := ifiAddress.Underlay.MarshalBinary()
	if err != nil {
		return nil, err
	}
	...
	s.logger.Tracef("handshake finished for peer (outbound) %s", remoteIfiAddress.Overlay.String())
	if len(resp.Ack.WelcomeMessage) > 0 {
		s.logger.Infof("greeting \"%s\" from peer: %s", resp.Ack.WelcomeMessage, remoteIfiAddress.Overlay.String())
	}
...
func (s *Service) Handle(ctx context.Context, stream p2p.Stream, remoteMultiaddr ma.Multiaddr, remotePeerID libp2ppeer.ID) (i *Info, err error) {
	...
	remoteIfiAddress, err := s.parseCheckAck(&ack)
	if err != nil {
		return nil, err
	}
...
func (s *Service) parseCheckAck(ack *pb.Ack) (*ifi.Address, error) {
	if ack.NetworkID != s.networkID {
		return nil, ErrNetworkIDIncompatible
	}

	ifiAddress, err := ifi.ParseAddress(ack.Address.Underlay, ack.Address.Overlay, ack.Address.Signature, s.networkID)
	if err != nil {
