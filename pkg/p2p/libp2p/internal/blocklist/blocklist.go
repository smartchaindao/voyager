...
func (b *Blocklist) Exists(overlay infinity.Address) (bool, error) {
...
func (b *Blocklist) Add(overlay infinity.Address, duration time.Duration) (err error) {
	key := generateKey(overlay)
	_, d, err := b.get(key)
	if err != nil {
		if err != storage.ErrNotFound {
...
func generateKey(overlay infinity.Address) string {
	return keyPrefix + overlay.String()
}

func unmarshalKey(s string) (infinity.Address, error) {
	addr := strings.TrimPrefix(s, keyPrefix)
	return infinity.ParseHexAddress(addr)
}
