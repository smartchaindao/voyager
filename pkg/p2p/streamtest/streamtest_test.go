...
func TestRecorder(t *testing.T) {
	...
	ask := func(ctx context.Context, s p2p.Streamer, address infinity.Address, questions ...string) (answers []string, err error) {
	...
	aa, err := ask(context.Background(), recorder, infinity.ZeroAddress, questions...)
	if err != nil {
		t.Fatal(err)
	}
	...
	_, err = recorder.Records(infinity.ZeroAddress, testProtocolName, testProtocolVersion, "invalid stream name")
...
func TestRecorder_errStreamNotSupported(t *testing.T) {
	...
	_, err := r.NewStream(context.Background(), infinity.ZeroAddress, nil, "testing", "messages", "1.0.1")
...
func TestRecorder_fullcloseWithRemoteClose(t *testing.T) {
	...
	request := func(ctx context.Context, s p2p.Streamer, address infinity.Address) (err error) {
		stream, err := s.NewStream(ctx, address, nil, testProtocolName, testProtocolVersion, testStreamName)
		if err != nil {
	...
	err := request(context.Background(), recorder, infinity.ZeroAddress)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRecorder_withMiddlewares(t *testing.T) {
	...
	request := func(ctx context.Context, s p2p.Streamer, address infinity.Address) error {
		stream, err := s.NewStream(ctx, address, nil, testProtocolName, testProtocolVersion, testStreamName)
		if err != nil {
			return fmt.Errorf("new stream: %w", err)
		}
