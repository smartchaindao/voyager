...
func WithBaseAddr(a infinity.Address) Option {
	return optionFunc(func(r *Recorder) {
		r.base = a
	})
}
...
func (r *Recorder) NewStream(ctx context.Context, addr infinity.Address, h p2p.Headers, protocolName, protocolVersion, streamName string) (p2p.Stream, error) {
	...
	id := addr.String() + p2p.NewInfinityStreamName(protocolName, protocolVersion, streamName)
...
func (r *Recorder) Records(addr infinity.Address, protocolName, protocolVersio, streamName string) ([]*Record, error) {
	id := addr.String() + p2p.NewInfinityStreamName(protocolName, protocolVersio, streamName)
...
func (r *Recorder) WaitRecords(t *testing.T, addr infinity.Address, proto, version, stream string, msgs, timeoutSec int) []*Record {
	t.Helper()
...
func (r *RecorderDisconnecter) Disconnect(overlay infinity.Address) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *RecorderDisconnecter) Blocklist(overlay infinity.Address, d time.Duration) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *RecorderDisconnecter) IsDisconnected(overlay infinity.Address) bool {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *RecorderDisconnecter) IsBlocklisted(overlay infinity.Address) (bool, time.Duration) {
	r.mu.RLock()
	defer r.mu.RUnlock()
