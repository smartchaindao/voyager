...
import (
	...
	"github.com/yanhuangpai/voyager/pkg/resolver/client/ens"
)
...
func TestENSntegration(t *testing.T) {
	...
	defaultAddr := infinity.MustParseHexAddress("00cb23598c2e520b6a6aae3ddc94fed4435a2909690bdd709bf9d9e7c2aadfad")
	...
	testCases := []struct {
		...
		{
			desc:    "not registered",
			name:    "unused.test.infinity.eth",
			wantErr: ens.ErrResolveFailed,
		},
		{
			desc:    "no content hash",
			name:    "nocontent.resolver.test.infinity.eth",
			wantErr: ens.ErrResolveFailed,
		},
		{
			desc:            "invalid contract address",
			contractAddress: "0xFFFFFFFF",
			name:            "example.resolver.test.infinity.eth",
			wantErr:         ens.ErrFailedToConnect,
		},
		{
			desc:    "ok",
