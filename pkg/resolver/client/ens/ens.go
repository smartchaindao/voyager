...
const (
	...
	infinityContentHashPrefix = "/infinity/"
)
...
var (
	...
	// not valid.
	ErrInvalidContentHash = errors.New("invalid Smart Chain content hash")
	// errNotImplemented denotes that the function has not voyagern implemented.
...
func (c *Client) Resolve(name string) (Address, error) {
	if c.resolveFn == nil {
		return infinity.ZeroAddress, fmt.Errorf("resolveFn: %w", errNotImplemented)
	}

	hash, err := c.resolveFn(c.registry, common.HexToAddress(c.contractAddr), name)
	if err != nil {
		return infinity.ZeroAddress, fmt.Errorf("%v: %w", err, ErrResolveFailed)
	}

	// Ensure that the content hash string is in a valid format, eg.
	// "/infinity/<address>".
	if !strings.HasPrefix(hash, infinityContentHashPrefix) {
		return infinity.ZeroAddress, fmt.Errorf("contenthash %s: %w", hash, ErrInvalidContentHash)
	}

	// Trim the prefix and try to parse the result as a ifi address.
