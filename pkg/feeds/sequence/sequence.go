...
func (f *finder) At(ctx context.Context, at, after int64) (ch infinity.Chunk, current, next feeds.Index, err error) {
	for i := uint64(0); ; i++ {
...
type result struct {
	chunk    infinity.Chunk // the chunk found
...
// At looks up the version valid at time `at`
// after is a unix time hint of the latest known update
func (f *asyncFinder) At(ctx context.Context, at, after int64) (ch infinity.Chunk, cur, next feeds.Index, err error) {
...
func (f *asyncFinder) get(ctx context.Context, at int64, idx uint64) (infinity.Chunk, error) {
