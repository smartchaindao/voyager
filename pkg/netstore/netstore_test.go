...
func TestNetstoreNoRetrieval(t *testing.T) {
	...
	_, err := store.Put(context.Background(), storage.ModePutUpload, infinity.NewChunk(addr, chunkData))
	if err != nil {
		t.Fatal(err)
	}
...
func (r *retrievalMock) RetrieveChunk(ctx context.Context, addr infinity.Address) (chunk infinity.Chunk, err error) {
	...
	r.addr = addr
	return infinity.NewChunk(addr, chunkData), nil
}
...
func (mr *mockRecovery) recovery(chunkAddress infinity.Address, targets pss.Targets) {
	mr.callbackC <- true
}

func (r *mockRecovery) RetrieveChunk(ctx context.Context, addr infinity.Address) (chunk infinity.Chunk, err error) {
	return nil, fmt.Errorf("chunk not found")
}
