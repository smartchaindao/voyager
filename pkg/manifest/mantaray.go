...
const (
	...
	ManifestMantarayContentType = "application/ifi-manifest-mantaray+octet-stream"
)
...
func (m *mantarayManifest) Lookup(ctx context.Context, path string) (Entry, error) {
	...
	address := infinity.NewAddress(node.Entry())
...
func (m *mantarayManifest) Store(ctx context.Context, storeSizeFn ...StoreSizeFunc) (infinity.Address, error) {
	...
	address := infinity.NewAddress(m.trie.Reference())

	return address, nil
}

func (m *mantarayManifest) IterateAddresses(ctx context.Context, fn infinity.AddressIterFunc) error {
	reference := infinity.NewAddress(m.trie.Reference())

	if infinity.ZeroAddress.Equal(reference) {
	...
	walker := func(path []byte, node *mantaray.Node, err error) error {
		...
		if node != nil {
			if node.Reference() != nil {
				ref := infinity.NewAddress(node.Reference())
			...
			if node.IsValueType() && node.Entry() != nil {
				entry := infinity.NewAddress(node.Entry())
				err = fn(entry)
				if err != nil {
					return err
				}
