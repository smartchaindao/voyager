...
const (
	...
	ManifestSimpleContentType = "application/ifi-manifest-simple+json"
)
...
func NewSimpleManifestReference(ref infinity.Address, l file.LoadSaver) (Interface, error) {
	m := &simpleManifest{
		manifest:  simple.NewManifest(),
		reference: ref,
		ls:        l,
...
func (m *simpleManifest) Lookup(_ context.Context, path string) (Entry, error) {
	...
	address, err := infinity.ParseHexAddress(n.Reference())
	if err != nil {
		return nil, fmt.Errorf("parse Smart Chain address: %w", err)
	}
...
func (m *simpleManifest) Store(ctx context.Context, storeSizeFn ...StoreSizeFunc) (infinity.Address, error) {
	data, err := m.manifest.MarshalBinary()
	if err != nil {
		return infinity.ZeroAddress, fmt.Errorf("manifest marshal error: %w", err)
	}
	...
	if len(storeSizeFn) > 0 {
		...
		for i := range storeSizeFn {
			...
			if err != nil {
				return infinity.ZeroAddress, fmt.Errorf("manifest store size func: %w", err)
			}
		}
	}
	...
	if err != nil {
		return infinity.ZeroAddress, fmt.Errorf("manifest save error: %w", err)
	}
	m.reference = infinity.NewAddress(ref)
	return m.reference, nil
}

func (m *simpleManifest) IterateAddresses(ctx context.Context, fn infinity.AddressIterFunc) error {
	if infinity.ZeroAddress.Equal(m.reference) {
	...
	walker := func(path string, entry simple.Entry, err error) error {
		...
		ref, err := infinity.ParseHexAddress(entry.Reference())
		if err != nil {
			return err
		}
