...
import (
	...
	mockSettlement "github.com/yanhuangpai/voyager/pkg/settlement/swap/mock"
...
func TestAccountingAddBalance(t *testing.T) {
	...
	peer2Addr, err := infinity.ParseHexAddress("00112244")
	if err != nil {
		t.Fatal(err)
	}
...
func (p *pricingMock) AnnouncePaymentThreshold(ctx context.Context, peer infinity.Address, paymentThreshold *big.Int) error {
	p.called = true
	p.peer = peer
