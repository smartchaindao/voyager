...
func TestTraversalBytes(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			dataSize:            infinity.ChunkSize * 128,
		...
		{
			dataSize:            infinity.ChunkSize * 129,
		...
		{
			dataSize:            infinity.ChunkSize*129 - 1,
			expectedHashesCount: 131,
			expectedHashes: []string{
	...
	for _, tc := range testCases {
		chunkCount := int(math.Ceil(float64(tc.dataSize) / infinity.ChunkSize))
...
func TestTraversalFiles(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			filesSize:           infinity.ChunkSize,
			contentType:         "text/plain; charset=utf-8",
			expectedHashesCount: 3,
			expectedHashes: []string{
				"29ae87fda18voyager4255ef19faabe901e2cf9c1c5c4648083383255670492e814e", // root
				"e7d4d4a897cd69f5759621044402e40a3d5c903cf1e225864eef5d1f77d97680",     // metadata
				"f833c17be12d68aec95eca7f9d993f7d7aaa7a9c282eb2c3d79ab26a5aeaf384",     // bytes (4096)
			},
		},
		{
			filesSize:           infinity.ChunkSize + 1,
			contentType:         "text/plain; charset=utf-8",
	...
	for _, tc := range testCases {
		chunkCount := int(math.Ceil(float64(tc.filesSize) / infinity.ChunkSize))
			...
			files: []file{
				{
					...
					dir:       "",
					name:      "hello.txt",
					reference: "a7c9250614bd2d2529e7voyager2e2d0df295661b7185465193dc3b54ffea30c4702",
					chunks: fileChunks{
...
func traversalCheck(t *testing.T,
	...
	err := traverseFn(traversalService)(
		ctx,
		reference,
		func(addr infinity.Address) error {
