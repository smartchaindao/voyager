...
type MockStorer struct {
	...
	pinnedAddress   []infinity.Address // Stores the pinned address
...
func WithBaseAddress(a infinity.Address) Option {
	return optionFunc(func(m *MockStorer) {
...
func NewStorer(opts ...Option) *MockStorer {
	s := &MockStorer{
		...
		bins:     make([]uint64, infinity.MaxBins),
	}
...
func (m *MockStorer) Get(_ context.Context, _ storage.ModeGet, addr infinity.Address) (ch infinity.Chunk, err error) {
	m.mtx.Lock()
	defer m.mtx.Unlock()
	...
	return infinity.NewChunk(addr, v), nil
}

func (m *MockStorer) Put(ctx context.Context, mode storage.ModePut, chs ...infinity.Chunk) (exist []bool, err error) {
	m.mtx.Lock()
	defer m.mtx.Unlock()
	...
	for i, ch := range chs {
		...
		if !exist[i] {
			po := infinity.Proximity(ch.Address().Bytes(), m.baseAddress)
			m.bins[po]++
		}
...
func (m *MockStorer) GetMulti(ctx context.Context, mode storage.ModeGet, addrs ...infinity.Address) (ch []infinity.Chunk, err error) {
	panic("not implemented") // TODO: Implement
}

func (m *MockStorer) has(ctx context.Context, addr infinity.Address) (yes bool, err error) {
...
func (m *MockStorer) HasMulti(ctx context.Context, addrs ...infinity.Address) (yes []bool, err error) {
	panic("not implemented") // TODO: Implement
}

func (m *MockStorer) Set(ctx context.Context, mode storage.ModeSet, addrs ...infinity.Address) (err error) {
	m.mtx.Lock()
	defer m.mtx.Unlock()
			...
			for i, ad := range m.pinnedAddress {
				if addr.String() == ad.String() {
					...
					if m.pinnedCounter[i] == 0 {
						...
						m.pinnedAddress[len(m.pinnedAddress)-1] = infinity.NewAddress([]byte{0})
...
func (m *MockStorer) GetModePut(addr infinity.Address) (mode storage.ModePut) {
	m.mtx.Lock()
	defer m.mtx.Unlock()
...
func (m *MockStorer) GetModeSet(addr infinity.Address) (mode storage.ModeSet) {
	m.mtx.Lock()
	defer m.mtx.Unlock()
...
func (m *MockStorer) SubscribePush(ctx context.Context) (c <-chan infinity.Chunk, stop func()) {
	panic("not implemented") // TODO: Implement
}
...
func (m *MockStorer) PinnedChunks(ctx context.Context, offset, cursor int) (pinnedChunks []*storage.Pinner, err error) {
	...
	for i, addr := range m.pinnedAddress {
		pi := &storage.Pinner{
			Address:    infinity.NewAddress(addr.Bytes()),
...
func (m *MockStorer) PinCounter(address infinity.Address) (uint64, error) {
	m.mtx.Lock()
	defer m.mtx.Unlock()
	for i, addr := range m.pinnedAddress {
