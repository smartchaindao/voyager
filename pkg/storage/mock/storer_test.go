...
func TestMockStorer(t *testing.T) {
	...
	keyFound, err := infinity.ParseHexAddress("aabbcc")
	if err != nil {
		t.Fatal(err)
	}
	keyNotFound, err := infinity.ParseHexAddress("bbccdd")
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := s.Put(ctx, storage.ModePutUpload, infinity.NewChunk(keyFound, valueFound)); err != nil {
