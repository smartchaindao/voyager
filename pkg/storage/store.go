...
// used across storage-aware components in Voyager.
package storage
...
type Storer interface {
	Getter
	Putter
	GetMulti(ctx context.Context, mode ModeGet, addrs ...infinity.Address) (ch []infinity.Chunk, err error)
	Hasser
	Setter
...
type Setter interface {
	Set(ctx context.Context, mode ModeSet, addrs ...infinity.Address) (err error)
}

type Hasser interface {
	Has(ctx context.Context, addr infinity.Address) (yes bool, err error)
	HasMulti(ctx context.Context, addrs ...infinity.Address) (yes []bool, err error)
}
