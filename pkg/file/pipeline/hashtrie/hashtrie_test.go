...
func TestLevels(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			rootch, err := s.Get(ctx, storage.ModeGetRequest, infinity.NewAddress(ref))
			if err != nil {
				t.Fatal(err)
			}

			//check the span. since write spans are 1 value 1, then expected span == tc.writes
			sp := binary.LittleEndian.Uint64(rootch.Data()[:infinity.SpanSize])
...
// described in https://github.com/yanhuangpai/voyager/issues/1175
