...
func TestSplitSingleChunk(t *testing.T) {
	...
	testData, err := g.SequentialBytes(infinity.ChunkSize)
	if err != nil {
		t.Fatal(err)
	}
	...
	testHashAddress := infinity.MustParseHexAddress(testHashHex)
...
func TestSplitThreeLevels(t *testing.T) {
	...
	g := mockbytes.New(0, mockbytes.MockTypeStandard).WithModulus(255)
	testData, err := g.SequentialBytes(infinity.ChunkSize * 128)
	if err != nil {
		t.Fatal(err)
	}
	...
	for i := 0; i < infinity.ChunkSize; i += infinity.SectionSize {
		dataAddressBytes := rootData[i : i+infinity.SectionSize]
		dataAddress := infinity.NewAddress(dataAddressBytes)
...
func TestUnalignedSplit(t *testing.T) {
	...
	var (
		dataLen       int64 = infinity.ChunkSize*2 + 32
	...
	doneC := make(chan infinity.Address)
	...
	writeSizes := []int{infinity.ChunkSize - 40, 40 + 32, infinity.ChunkSize}
	...
	select {
	case addr := <-doneC:
		expectAddr := infinity.MustParseHexAddress(expectAddrHex)
...
goos: linux
goarch: amd64
pkg: github.com/yanhuangpai/voyager/pkg/file/splitter
BenchmarkSplitter
...
PASS
ok  	github.com/yanhuangpai/voyager/pkg/file/splitter	22.476s
*/
