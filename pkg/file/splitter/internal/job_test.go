...
func TestSplitterJobPartialSingleChunk(t *testing.T) {
	...
	addressResult := infinity.NewAddress(hashResult)
...
func testSplitterJobVector(t *testing.T) {
	...
	for i := 0; i < len(data); i += infinity.ChunkSize {
		l := infinity.ChunkSize
		if len(data)-i < infinity.ChunkSize {
			l = len(data) - i
		}
	...
	actual := infinity.NewAddress(actualBytes)
