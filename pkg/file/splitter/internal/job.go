...
type Putter interface {
	Put(context.Context, infinity.Chunk) ([]bool, error)
}
...
// until the full data length has voyagern written. The Sum should be called which will
// return the InfinityHash of the data.
//
// Called Sum before the last Write, or Write after Sum has voyagern called, may result in
...
func NewSimpleSplitterJob(ctx context.Context, putter Putter, spanLength int64, toEncrypt bool) *SimpleSplitterJob {
	...
	return &SimpleSplitterJob{
		...
		buffer:     make([]byte, infinity.ChunkWithSpanSize*levelBufferLimit*2), // double size as temp workaround for weak calculation of needed buffer space
		tag:        sctx.GetTag(ctx),
		toEncrypt:  toEncrypt,
		refSize:    refSize,
	}
...
func (j *SimpleSplitterJob) Write(b []byte) (int, error) {
	if len(b) > infinity.ChunkSize {
		return 0, fmt.Errorf("Write must be called with a maximum of %d bytes", infinity.ChunkSize)
	}
...
// Sum returns the Smart Chain hash of the data.
...
func (s *SimpleSplitterJob) writeToLevel(lvl int, data []byte) error {
	...
	if s.cursors[lvl]-s.cursors[lvl+1] == infinity.ChunkSize {
...
func (s *SimpleSplitterJob) sumLevel(lvl int) ([]byte, error) {
	s.sumCounts[lvl]++
	spanSize := file.Spans[lvl] * infinity.ChunkSize
	span := (s.length-1)%spanSize + 1

	var chunkData []byte

	head := make([]byte, infinity.SpanSize)
...
//
// The method does not check that the final hash actually has voyagern written, so
// timing is the responsibility of the caller.
func (s *SimpleSplitterJob) digest() []byte {
	if s.toEncrypt {
		return s.buffer[:infinity.SectionSize*2]
	} else {
		return s.buffer[:infinity.SectionSize]
	}
}
...
func (s *SimpleSplitterJob) hashUnfinished() error {
	if s.length%infinity.ChunkSize != 0 {
...
func (s *SimpleSplitterJob) moveDanglingChunk() error {
	...
	targetLevel := file.Levels(s.length, infinity.SectionSize, infinity.Branches)
...
func (s *SimpleSplitterJob) newSpanEncryption(key encryption.Key) encryption.Interface {
	return encryption.New(key, 0, uint32(infinity.ChunkSize/s.refSize), sha3.NewLegacyKeccak256)
}
