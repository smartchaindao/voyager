...
import (
	...
	"github.com/yanhuangpai/voyager/pkg/encryption/store"
...
func TestJoiner_ErrReferenceLength(t *testing.T) {
	...
	_, _, err := joiner.New(context.Background(), store, infinity.ZeroAddress)

	if !errors.Is(err, storage.ErrReferenceLength) {
		t.Fatalf("expected ErrReferenceLength %x but got %v", infinity.ZeroAddress, err)
	}
}
...
func TestJoinerSingleChunk(t *testing.T) {
	...
	mockAddr := infinity.MustParseHexAddress(mockAddrHex)
	mockData := []byte("foo")
	mockDataLengthBytes := make([]byte, 8)
	mockDataLengthBytes[0] = 0x03
	mockChunk := infinity.NewChunk(mockAddr, append(mockDataLengthBytes, mockData...))
...
func TestJoinerWithReference(t *testing.T) {
	...
	if l != int64(infinity.ChunkSize*2) {
		t.Fatalf("expected join data length %d, got %d", infinity.ChunkSize*2, l)
	}

	resultBuffer := make([]byte, infinity.ChunkSize)
...
func TestSeek(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			name: "one chunk",
			size: infinity.ChunkSize,
		},
		{
			name: "a few chunks",
			size: 10 * infinity.ChunkSize,
		},
		{
			name: "a few chunks and a change",
			size: 10*infinity.ChunkSize + 84,
		},
		{
			name: "a few chunks more",
			size: 2*infinity.ChunkSize*infinity.ChunkSize + 1000,
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			validateRead := func(t *testing.T, name string, i int) {
				...
				got := make([]byte, infinity.ChunkSize)
...
func TestJoinerReadAt(t *testing.T) {
	...
	b := make([]byte, infinity.ChunkSize)
	_, err = j.ReadAt(b, infinity.ChunkSize)
	if err != nil {
		t.Fatal(err)
	}
...
func TestJoinerOneLevel(t *testing.T) {
	...
	outBuffer := make([]byte, infinity.ChunkSize)
	c, err := j.Read(outBuffer)
	if err != nil {
		t.Fatal(err)
	}
	if c != infinity.ChunkSize {
		t.Fatalf("expected firstchunk read count %d, got %d", infinity.ChunkSize, c)
	}
	...
	if c != infinity.ChunkSize {
		t.Fatalf("expected secondchunk read count %d, got %d", infinity.ChunkSize, c)
	}
...
func TestJoinerTwoLevelsAcrossChunk(t *testing.T) {
	...
	rootChunk := filetest.GenerateTestRandomFileChunk(infinity.ZeroAddress, infinity.ChunkSize*infinity.Branches+42, infinity.SectionSize*2)
	_, err := store.Put(ctx, storage.ModePutUpload, rootChunk)
	if err != nil {
		t.Fatal(err)
	}
	...
	firstAddress := infinity.NewAddress(rootChunk.Data()[8 : infinity.SectionSize+8])
	firstChunk := filetest.GenerateTestRandomFileChunk(firstAddress, infinity.ChunkSize*infinity.Branches, infinity.ChunkSize)
	_, err = store.Put(ctx, storage.ModePutUpload, firstChunk)
	if err != nil {
		t.Fatal(err)
	}
	...
	secondAddress := infinity.NewAddress(rootChunk.Data()[infinity.SectionSize+8:])
	secondChunk := filetest.GenerateTestRandomFileChunk(secondAddress, 42, infinity.SectionSize)
	_, err = store.Put(ctx, storage.ModePutUpload, secondChunk)
	if err != nil {
		t.Fatal(err)
	}
	...
	cursor := 8
	for i := 0; i < infinity.Branches; i++ {
		chunkAddressBytes := firstChunk.Data()[cursor : cursor+infinity.SectionSize]
		chunkAddress := infinity.NewAddress(chunkAddressBytes)
		ch := filetest.GenerateTestRandomFileChunk(chunkAddress, infinity.ChunkSize, infinity.ChunkSize)
		_, err := store.Put(ctx, storage.ModePutUpload, ch)
		if err != nil {
			t.Fatal(err)
		}
		cursor += infinity.SectionSize
	}
	...
	for i := 0; i < infinity.Branches; i++ {
		...
		if c != infinity.ChunkSize {
			t.Fatalf("chunk %d expected read %d bytes; got %d", i, infinity.ChunkSize, c)
		}
	}
	c, err := j.Read(b)
	if err != nil {
...
func TestJoinerIterateChunkAddresses(t *testing.T) {
	...
	err = j.IterateChunkAddresses(func(addr infinity.Address) error {
