...
func Service(t *testing.T, s keystore.Service) {
	...
	// create a new Smart Chain key
	k1, created, err := s.Key("infinity", "pass123456")
	if err != nil {
		t.Fatal(err)
	}
	if !created {
	...
	// get Smart Chain key
	k2, created, err := s.Key("infinity", "pass123456")
	if err != nil {
		t.Fatal(err)
	}
	if created {
	...
	// invalid password
	_, _, err = s.Key("infinity", "invalid password")
