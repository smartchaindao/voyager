...
import (
	...
	pb "github.com/yanhuangpai/voyager/pkg/settlement/pseudosettle/pb"
...
func totalKey(peer infinity.Address, prefix string) string {
	return fmt.Sprintf("%v%v", prefix, peer.String())
}

func totalKeyPeer(key []byte, prefix string) (peer infinity.Address, err error) {
	k := string(key)

	split := strings.SplitAfter(k, prefix)
	if len(split) != 2 {
		return infinity.ZeroAddress, errors.New("no peer in key")
	}
	return infinity.ParseHexAddress(split[1])
}
