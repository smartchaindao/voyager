...
func storeDir(ctx context.Context, encrypt bool, reader io.ReadCloser, log logging.Logger, p pipelineFunc, ls file.LoadSaver, indexFilename string, errorFilename string, tag *tags.Tag, tagCreated bool) (infinity.Address, error) {
	...
	if indexFilename != "" && strings.ContainsRune(indexFilename, '/') {
		return infinity.ZeroAddress, fmt.Errorf("index document suffix must not include slash character")
	}
	...
	for {
		...
		if err == io.EOF {
			...
			return infinity.ZeroAddress, fmt.Errorf("read tar stream: %w", err)
		}
		...
		if !tagCreated {
			...
			if estimatedTotalChunks := calculateNumberOfChunks(fileInfo.size, encrypt); estimatedTotalChunks > 0 {
				...
				if err != nil {
					return infinity.ZeroAddress, fmt.Errorf("increment tag: %w", err)
				}
			}
		}
		...
		if err != nil {
			return infinity.ZeroAddress, fmt.Errorf("store dir file: %w", err)
		}
	...
	if filesAdded == 0 {
		return infinity.ZeroAddress, fmt.Errorf("no files in tar")
	}
	...
	if err != nil {
		return infinity.ZeroAddress, fmt.Errorf("store manifest: %w", err)
	}
	...
	if err != nil {
		return infinity.ZeroAddress, fmt.Errorf("metadata marshal: %w", err)
	}
	...
	if err != nil {
		return infinity.ZeroAddress, fmt.Errorf("split metadata: %w", err)
	}
	...
	if err != nil {
		return infinity.ZeroAddress, fmt.Errorf("entry marshal: %w", err)
	}

	manifestFileReference, err := p(ctx, bytes.NewReader(fileEntryBytes), int64(len(fileEntryBytes)))
	if err != nil {
		return infinity.ZeroAddress, fmt.Errorf("split entry: %w", err)
	}
...
func storeFile(ctx context.Context, fileInfo *fileUploadInfo, p pipelineFunc, encrypt bool, tag *tags.Tag, tagCreated bool) (infinity.Address, error) {
	// first store the file and get its reference
	fr, err := p(ctx, fileInfo.reader, fileInfo.size)
	if err != nil {
		return infinity.ZeroAddress, fmt.Errorf("split file: %w", err)
	}
