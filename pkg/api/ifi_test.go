...
import (
	...
	smock "github.com/yanhuangpai/voyager/pkg/storage/mock"
	"github.com/yanhuangpai/voyager/pkg/tags"
)

func TestIfi(t *testing.T) {
	var (
		...
		pipeWriteAll = func(r io.Reader, l int64) (infinity.Address, error) {
	...
	t.Run("download-file-by-path", func(t *testing.T) {
		...
		var err error
		var fileContentReference infinity.Address
		var fileReference infinity.Address
		var manifestFileReference infinity.Address
		...
		rcvdHeader := jsonhttptest.Request(t, client, http.MethodGet, ifiDownloadResource(manifestFileReference.String(), filePath), http.StatusOK,
		...
		jsonhttptest.Request(t, client, http.MethodGet, ifiDownloadResource(manifestFileReference.String(), missingFilePath), http.StatusNotFound,
...
func TestFeedIndirection(t *testing.T) {
	...
	var (
		updateData     = []byte("<h1>Smart Chain Feeds Hello World!</h1>")
		mockStatestore = statestore.NewStateStore()
		logger         = logging.New(ioutil.Discard, 0)
		storer         = smock.NewStorer()
		client, _, _   = newTestServer(t, testServerOptions{
	...
	options := []jsonhttptest.Option{
		...
		jsonhttptest.WithRequestHeader(api.InfinityIndexDocumentHeader, "index.html"),
	}
	...
	// called from the ifi endpoint. then call the ifi endpoint with
	// the pregenerated feed root manifest hash

	feedUpdate := toChunk(t, 121212, resp.Reference.Bytes())

	var (
		feedChunkAddr       = infinity.MustParseHexAddress("891a1d1c8436c792d02fc2e8883fef7ab387eaeaacd25aa9f518be7be7856d54")
		...
		manifestCh          = infinity.NewChunk(infinity.MustParseHexAddress("8504f2a107ca940beafc4ce2f6c9a9f0968c62a5b5893ff0e4e1e2983048d276"), chData)
	...
	_, err := storer.Put(ctx, storage.ModePutUpload, infinity.NewChunk(feedChunkAddr, feedChunkData))
	if err != nil {
		t.Fatal(err)
	}
	...
	jsonhttptest.Request(t, client, http.MethodGet, ifiDownloadResource(feedChunkAddr.String(), ""), http.StatusOK,
