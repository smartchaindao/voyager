...
func TestTags(t *testing.T) {
	...
	t.Run("create tag with invalid id", func(t *testing.T) {
		jsonhttptest.Request(t, client, http.MethodPost, chunksResource, http.StatusBadRequest,
			...
			jsonhttptest.WithRequestHeader(api.InfinityTagHeader, "invalid_id.jpg"), // the value should be uint32
		)
	})
	...
	t.Run("create tag upload chunk", func(t *testing.T) {
		...
		rcvdHeaders := jsonhttptest.Request(t, client, http.MethodPost, chunksResource, http.StatusOK,
			jsonhttptest.WithRequestBody(bytes.NewReader(chunk.Data())),
			jsonhttptest.WithExpectedJSONResponse(api.ChunkAddressResponse{Reference: chunk.Address()}),
			jsonhttptest.WithRequestHeader(api.InfinityTagHeader, strconv.FormatUint(uint64(tr.Uid), 10)),
		)

		isTagFoundInResponse(t, rcvdHeaders, &tr)
		tagValueTest(t, tr.Uid, 1, 1, 1, 0, 0, 0, infinity.ZeroAddress, client)
	})
	...
	t.Run("done split", func(t *testing.T) {
		...
		jsonhttptest.Request(t, client, http.MethodPost, chunksResource, http.StatusOK,
			jsonhttptest.WithRequestBody(bytes.NewReader(chunk.Data())),
			jsonhttptest.WithRequestHeader(api.InfinityTagHeader, fmt.Sprint(tagId)),
		)
	...
	t.Run("file tags", func(t *testing.T) {
		...
		expectedHash := infinity.MustParseHexAddress("8e27bb803ff049e8c2f4650357026723220170c15ebf9b635a7026539879a1a8")
		...
		tagId, err := strconv.Atoi(respHeaders.Get(api.InfinityTagHeader))
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("dir tags", func(t *testing.T) {
		...
		expectedHash := infinity.MustParseHexAddress("3dc643abeb3db60a4dfb72008b577dd9a573abaa74c6afe37a75c63ceea829f6")
		expectedResponse := api.FileUploadResponse{Reference: expectedHash}
	...
	t.Run("bytes tags", func(t *testing.T) {
		...
		sentHeaders.Set(api.InfinityTagHeader, strconv.FormatUint(uint64(tr.Uid), 10))

		g := mockbytes.New(0, mockbytes.MockTypeStandard).WithModulus(255)
		dataChunk, err := g.SequentialBytes(infinity.ChunkSize)
		if err != nil {
			t.Fatal(err)
		}

		rootAddress := infinity.MustParseHexAddress("5e2a21902f51438be1adbd0e29e1bd34c53a21d3120aefa3c7275129f2f88de9")

		content := make([]byte, infinity.ChunkSize*2)
		copy(content[infinity.ChunkSize:], dataChunk)
		copy(content[:infinity.ChunkSize], dataChunk)
		...
		tagValueTest(t, id, 3, 3, 1, 0, 0, 3, infinity.ZeroAddress, client)
	})
}
...
func isTagFoundInResponse(t *testing.T, headers http.Header, tr *api.TagResponse) uint32 {
	...
	idStr := headers.Get(api.InfinityTagHeader)
	if idStr == "" {
