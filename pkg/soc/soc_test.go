...
func TestNew(t *testing.T) {
	...
	spanBytes := make([]byte, infinity.SpanSize)
	binary.LittleEndian.PutUint64(spanBytes, uint64(len(payload)))
	if !bytes.Equal(chunkData[:infinity.SpanSize], spanBytes) {
		t.Fatalf("span mismatch. got %x want %x", chunkData[:infinity.SpanSize], spanBytes)
	}

	if !bytes.Equal(chunkData[infinity.SpanSize:], payload) {
		t.Fatalf("payload mismatch. got %x want %x", chunkData[infinity.SpanSize:], payload)
	}
}
...
func TestChunk(t *testing.T) {
	...
	expectedSOCAddress := infinity.NewAddress(sum)
	...
	spanBytes := make([]byte, infinity.SpanSize)
	binary.LittleEndian.PutUint64(spanBytes, uint64(len(payload)))
	if !bytes.Equal(chunkData[cursor:cursor+infinity.SpanSize], spanBytes) {
		t.Fatalf("span mismatch. got %x want %x", chunkData[cursor:cursor+infinity.SpanSize], spanBytes)
	}
	cursor += infinity.SpanSize
...
func TestFromChunk(t *testing.T) {
	...
	chunkAddress := infinity.MustParseHexAddress("2387e8e7d8a48c2a9339c97c1dc3461a9a7aa07e994c5cb8b38fd7c1b3e6ea48")
	ch := infinity.NewChunk(chunkAddress, chunkData)
