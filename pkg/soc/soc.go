...
const (
	IdSize        = 32
	SignatureSize = 65
	minChunkSize  = IdSize + SignatureSize + infinity.SpanSize
)
...
type SOC struct {
	...
	signature []byte
	chunk     infinity.Chunk // wrapped chunk.
}

// New creates a new SOC representation from arbitrary id and
// a content-addressed chunk.
func New(id ID, ch infinity.Chunk) *SOC {
	return &SOC{
		id:    id,
		chunk: ch,
	}
...
func NewSigned(id ID, ch infinity.Chunk, owner, sig []byte) (*SOC, error) {
	s := New(id, ch)
...
func (s *SOC) address() (infinity.Address, error) {
	if len(s.owner) != crypto.AddressSize {
		return infinity.ZeroAddress, errInvalidAddress
	}
	return CreateAddress(s.id, s.owner)
}

// WrappedChunk returns the chunk wrapped by the SOC.
func (s *SOC) WrappedChunk() infinity.Chunk {
	return s.chunk
}

// Chunk returns the SOC chunk.
func (s *SOC) Chunk() (infinity.Chunk, error) {
	socAddress, err := s.address()
	if err != nil {
		return nil, err
	}
	return infinity.NewChunk(socAddress, s.toBytes()), nil
}
...
func (s *SOC) Sign(signer crypto.Signer) (infinity.Chunk, error) {
	// create owner
...
// FromChunk recreates a SOC representation from infinity.Chunk data.
func FromChunk(sch infinity.Chunk) (*SOC, error) {
	chunkData := sch.Data()
