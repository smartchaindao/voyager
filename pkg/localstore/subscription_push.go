...
func (db *DB) SubscribePush(ctx context.Context) (c <-chan infinity.Chunk, stop func()) {
	db.metrics.SubscribePush.Inc()

	chunks := make(chan infinity.Chunk)
		...
		for {
			...
			case <-out:
				...
				err := db.pushIndex.Iterate(func(item shed.Item) (stop bool, err error) {
					...
					select {
					case chunks <- infinity.NewChunk(infinity.NewAddress(dataItem.Address), dataItem.Data).WithTagID(item.Tag):
						count++
