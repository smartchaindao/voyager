...
func TestModePutUpload_parallel(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			chunksChan := make(chan []infinity.Chunk)
...
//
// # go test -benchmem -run=none github.com/yanhuangpai/infinity/storage/localstore -bench BenchmarkPutUpload -v
//
// goos: darwin
// goarch: amd64
