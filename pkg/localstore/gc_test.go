...
func TestPinGC(t *testing.T) {
	...
	addrs := make([]infinity.Address, 0)
	pinAddrs := make([]infinity.Address, 0)
...
func TestPinAfterMultiGC(t *testing.T) {
	...
	pinnedChunks := make([]infinity.Address, 0)
	...
	for _, addr := range pinnedChunks {
		...
		gotChunk, err := db.Get(context.Background(), storage.ModeGetRequest, infinity.NewAddress(outItem.Address))
		if err != nil {
			t.Fatal(err)
		}
		if !gotChunk.Address().Equal(infinity.NewAddress(addr.Bytes())) {
...
func generateAndPinAChunk(t *testing.T, db *DB) infinity.Chunk {
...
func addRandomChunks(t *testing.T, count int, db *DB, pin bool) []infinity.Chunk {
...
func TestGC_NoEvictDirty(t *testing.T) {
	...
	t.Cleanup(setTestHookCollectGarbage(func(collectedCount uint64) {
		...
		// before the correct signal has voyagern communicated here.
		if collectedCount == 0 {
			return
		}
		select {
