...
type DB struct {
	...
	dirtyAddresses []infinity.Address
...
func New(path string, baseKey []byte, o *Options, logger logging.Logger) (db *DB, err error) {
	...
	db.pullIndex, err = db.shed.NewIndex("PO|BinID->Hash|Tag", shed.IndexFuncs{
		EncodeKey: func(fields shed.Item) (key []byte, err error) {
			key = make([]byte, 41)
			key[0] = db.po(infinity.NewAddress(fields.Address))
...
func (db *DB) po(addr infinity.Address) (bin uint8) {
	return infinity.Proximity(db.baseKey, addr.Bytes())
}
...
func chunkToItem(ch infinity.Chunk) shed.Item {
	return shed.Item{
...
func addressToItem(addr infinity.Address) shed.Item {
	return shed.Item{
		Address: addr.Bytes(),
	}
}
...
func addressesToItems(addrs ...infinity.Address) []shed.Item {
