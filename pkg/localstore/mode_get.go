...
// interface.
func (db *DB) Get(ctx context.Context, mode storage.ModeGet, addr infinity.Address) (ch infinity.Chunk, err error) {
	...
	return infinity.NewChunk(infinity.NewAddress(out.Address), out.Data).WithPinCounter(out.PinCounter), nil
}

// get returns Item from the retrieval index
// and updates other indexes.
func (db *DB) get(mode storage.ModeGet, addr infinity.Address) (out shed.Item, err error) {
...
func (db *DB) updateGC(item shed.Item) (err error) {
	...
	if db.gcRunning {
		db.dirtyAddresses = append(db.dirtyAddresses, infinity.NewAddress(item.Address))
	}
