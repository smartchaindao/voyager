...
func TestShallowestEmpty(t *testing.T) {
	var (
		...
		peers = make([][]infinity.Address, 16)
	)
	...
	for _, tc := range []struct {
		...
		for _, v := range peers[tc.removePo] {
			po := infinity.Proximity(base.Bytes(), v.Bytes())
			ps.Remove(v, po)
		}
		sd, none := ps.ShallowestEmpty()
...
func TestAddRemove(t *testing.T) {
	...
	chkNotExists(t, ps, append([]infinity.Address{peers[1]}, peers[4:]...)...)
	...
	chkNotExists(t, ps, append([]infinity.Address{peers[1]}, peers[4:7]...)...)

	ps.Add(peers[5], 2)
	chkLen(t, ps, 5)
	chkBins(t, ps, []uint{0, 1, 3, 4})
	chkExists(t, ps, peers[0], peers[2], peers[3], peers[5], peers[7])
	chkNotExists(t, ps, []infinity.Address{peers[1], peers[4], peers[6]}...)

	ps.Remove(peers[2], 1)
	chkLen(t, ps, 4)
	chkBins(t, ps, []uint{0, 1, 2, 3})
	chkExists(t, ps, peers[0], peers[3], peers[5], peers[7])
	chkNotExists(t, ps, []infinity.Address{peers[1], peers[2], peers[4], peers[6]}...)
...
func TestIteratorError(t *testing.T) {
	...
	f := func(p infinity.Address, _ uint8) (stop, jumpToNext bool, err error) {
...
func TestIterators(t *testing.T) {
	...
	base := test.RandomAddress()
	peers := make([]infinity.Address, 4)
	for i := 0; i < 4; i++ {
		a := test.RandomAddressAt(base, i)
		peers[i] = a
	}

	testIterator(t, ps, false, false, 0, []infinity.Address{})
	testIteratorRev(t, ps, false, false, 0, []infinity.Address{})

	for i, v := range peers {
		ps.Add(v, uint8(i))
	}

	testIterator(t, ps, false, false, 4, []infinity.Address{peers[3], peers[2], peers[1], peers[0]})
	testIteratorRev(t, ps, false, false, 4, peers)

	ps.Remove(peers[2], 2)
	testIterator(t, ps, false, false, 3, []infinity.Address{peers[3], peers[1], peers[0]})
	testIteratorRev(t, ps, false, false, 3, []infinity.Address{peers[0], peers[1], peers[3]})

	ps.Remove(peers[0], 0)
	testIterator(t, ps, false, false, 2, []infinity.Address{peers[3], peers[1]})
	testIteratorRev(t, ps, false, false, 2, []infinity.Address{peers[1], peers[3]})

	ps.Remove(peers[3], 3)
	testIterator(t, ps, false, false, 1, []infinity.Address{peers[1]})
	testIteratorRev(t, ps, false, false, 1, []infinity.Address{peers[1]})
...
func TestIteratorsJumpStop(t *testing.T) {
	...
	base := test.RandomAddress()
	peers := make([]infinity.Address, 12)
	j := 0
	for i := 0; i < 4; i++ {
		for ii := 0; ii < 3; ii++ {
			a := test.RandomAddressAt(base, i)
	...
	testIterator(t, ps, true, false, 4, []infinity.Address{peers[11], peers[8], peers[5], peers[2]})
	testIteratorRev(t, ps, true, false, 4, []infinity.Address{peers[2], peers[5], peers[8], peers[11]})

	// check that the stop functionality works correctly
	testIterator(t, ps, true, true, 1, []infinity.Address{peers[11]})
	testIteratorRev(t, ps, true, true, 1, []infinity.Address{peers[2]})

}

func testIteratorRev(t *testing.T, ps *pslice.PSlice, skipNext, stop bool, iterations int, peerseq []infinity.Address) {
	t.Helper()
	i := 0
	f := func(p infinity.Address, po uint8) (bool, bool, error) {
		if i == iterations {
...
func testIterator(t *testing.T, ps *pslice.PSlice, skipNext, stop bool, iterations int, peerseq []infinity.Address) {
	t.Helper()
	i := 0
	f := func(p infinity.Address, po uint8) (bool, bool, error) {
		if i == iterations {
...
func chkExists(t *testing.T, ps *pslice.PSlice, addrs ...infinity.Address) {
	t.Helper()
	for _, a := range addrs {
		if !ps.Exists(a) {
...
func chkNotExists(t *testing.T, ps *pslice.PSlice, addrs ...infinity.Address) {
	t.Helper()
	for _, a := range addrs {
		if ps.Exists(a) {
