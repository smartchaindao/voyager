...
import (
	...
	stesting "github.com/yanhuangpai/voyager/pkg/storage/testing"
)

var (
	addrs = []infinity.Address{
		infinity.MustParseHexAddress("0001"),
		infinity.MustParseHexAddress("0002"),
		infinity.MustParseHexAddress("0003"),
		infinity.MustParseHexAddress("0004"),
		infinity.MustParseHexAddress("0005"),
		infinity.MustParseHexAddress("0006"),
	}
	limit = 5
)

func someAddrs(i ...int) (r []infinity.Address) {
	for _, v := range i {
		r = append(r, addrs[v])
	}
	return r
...
func TestIntervalChunks_Localstore(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			var chunks []infinity.Chunk

			for i := 1; i <= tc.chunks; {
				c := stesting.GenerateTestRandomChunk()
				po := infinity.Proximity(c.Address().Bytes(), base)
				if po == 1 {
			...
			checkAddrs := make([]infinity.Address, len(tc.addrs))
...
func checkAinB(t *testing.T, a, b []infinity.Address) {
	t.Helper()
	for _, v := range a {
		if !isIn(v, b) {
...
func isIn(a infinity.Address, b []infinity.Address) bool {
	for _, v := range b {
		if a.Equal(v) {
			return true
		}
