...
type PullSyncMock struct {
	...
	syncCalls       []SyncCall
	cursors         []uint64
	getCursorsPeers []infinity.Address
	autoReply       bool
	blockLiveSync   bool
...
func (p *PullSyncMock) SyncInterval(ctx context.Context, peer infinity.Address, bin uint8, from, to uint64) (topmost uint64, ruid uint32, err error) {
...
func (p *PullSyncMock) GetCursors(_ context.Context, peer infinity.Address) ([]uint64, error) {
	p.mtx.Lock()
	defer p.mtx.Unlock()
...
func (p *PullSyncMock) SyncCalls(peer infinity.Address) (res []SyncCall) {
	p.mtx.Lock()
	defer p.mtx.Unlock()
...
func (p *PullSyncMock) CancelRuid(ctx context.Context, peer infinity.Address, ruid uint32) error {
	return nil
}

func (p *PullSyncMock) LiveSyncCalls(peer infinity.Address) (res []SyncCall) {
	p.mtx.Lock()
	defer p.mtx.Unlock()
...
func (p *PullSyncMock) CursorsCalls(peer infinity.Address) bool {
	p.mtx.Lock()
	defer p.mtx.Unlock()
