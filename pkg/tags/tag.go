...
const (
	...
	StateSplit               // chunk has voyagern processed by filehasher/Smart Chain safe call
...
// DoneSplit sets total count to SPLIT count and sets the associated Smart Chain hash for this tag
// is meant to be called when splitter finishes for input streams of unknown size
func (t *Tag) DoneSplit(address infinity.Address) (int64, error) {
	total := atomic.LoadInt64(&t.Split)
	atomic.StoreInt64(&t.Total, total)

	if !address.Equal(infinity.ZeroAddress) {
		t.Address = address
	}
