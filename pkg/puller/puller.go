...
func (p *Puller) manage() {
	...
	for {
		...
		case <-c:
			...
			_ = p.topology.EachPeerRev(func(peerAddr infinity.Address, po uint8) (stop, jumpToNext bool, err error) {
				bp := p.syncPeers[po]
...
func (p *Puller) disconnectPeer(ctx context.Context, peer infinity.Address, po uint8) {
	if logMore {
...
func (p *Puller) recalcPeer(ctx context.Context, peer infinity.Address, po, d uint8) {
	if logMore {
...
func (p *Puller) syncPeer(ctx context.Context, peer infinity.Address, po, d uint8) {
...
func (p *Puller) syncPeerBin(ctx context.Context, syncCtx *syncPeer, peer infinity.Address, bin uint8, cur uint64) {
...
func (p *Puller) histSyncWorker(ctx context.Context, peer infinity.Address, bin uint8, cur uint64) {
	defer func() {
		p.wg.Done()
...
func (p *Puller) liveSyncWorker(ctx context.Context, peer infinity.Address, bin uint8, cur uint64) {
	defer p.wg.Done()
	if logMore {
...
func (p *Puller) addPeerInterval(peer infinity.Address, bin uint8, start, end uint64) (err error) {
	p.intervalMtx.Lock()
...
func (p *Puller) nextPeerInterval(peer infinity.Address, bin uint8) (start, end uint64, empty bool, err error) {
	p.intervalMtx.Lock()
	defer p.intervalMtx.Unlock()
...
func (p *Puller) getOrCreateInterval(peer infinity.Address, bin uint8) (*intervalstore.Intervals, error) {
	p.mtx.Lock()
	defer p.mtx.Unlock()
...
func peerIntervalKey(peer infinity.Address, bin uint8) string {
...
func newSyncPeer(addr infinity.Address, bins uint8) *syncPeer {
	return &syncPeer{
		address:        addr,
...
func isSyncing(p *Puller, addr infinity.Address) bool {
