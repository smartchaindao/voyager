...
import (
	...
	mockk "github.com/yanhuangpai/voyager/pkg/kademlia/mock"
	"github.com/yanhuangpai/voyager/pkg/logging"
	"github.com/yanhuangpai/voyager/pkg/puller"
	mockps "github.com/yanhuangpai/voyager/pkg/pullsync/mock"
...
func checkIntervals(t *testing.T, s storage.StateStorer, addr infinity.Address, expInterval string, bin uint8) {
	t.Helper()
...
func checkNotFound(t *testing.T, s storage.StateStorer, addr infinity.Address, bin uint8) {
	t.Helper()
	key := puller.PeerIntervalKey(addr, bin)
	i := &intervalstore.Intervals{}
	err := s.Get(key, i)
...
func waitCheckCalls(t *testing.T, expCalls []c, callsFn func(infinity.Address) []mockps.SyncCall, addr infinity.Address) {
	t.Helper()
	for i := 0; i < 10; i++ {
		time.Sleep(50 * time.Millisecond)
		calls := callsFn(addr)
...
func waitCursorsCalled(t *testing.T, ps *mockps.PullSyncMock, addr infinity.Address, invert bool) {
	t.Helper()
	for i := 0; i < 20; i++ {
...
func waitLiveSyncCalled(t *testing.T, ps *mockps.PullSyncMock, addr infinity.Address, invert bool) {
	t.Helper()
	for i := 0; i < 15; i++ {
...
func waitSyncCalled(t *testing.T, ps *mockps.PullSyncMock, addr infinity.Address, invert bool) {
	t.Helper()
	for i := 0; i < 15; i++ {
		v := ps.SyncCalls(addr)
		if len(v) > 0 {
...
func waitSyncCalledTimes(t *testing.T, ps *mockps.PullSyncMock, addr infinity.Address, times int) {
	t.Helper()
	for i := 0; i < 15; i++ {
		v := ps.SyncCalls(addr)
		if len(v) == times {
...
func waitLiveSyncCalledTimes(t *testing.T, ps *mockps.PullSyncMock, addr infinity.Address, times int) {
	t.Helper()
	for i := 0; i < 15; i++ {
		v := ps.LiveSyncCalls(addr)
		if len(v) == times {
